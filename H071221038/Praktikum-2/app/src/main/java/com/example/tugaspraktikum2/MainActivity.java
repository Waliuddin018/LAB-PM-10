        ...
        ActivityResultLauncher<Intent> intentLaunch = registerForActivityResult(
                new ActivityResultContracts.StartActivityForResult(), result -> {
                    if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                        ...
                        try {
                            InputStream inputStream = getContentResolver().openInputStream(uriGambar); // terima gambar dari urigambar
                            Bitmap bitmap = BitmapFactory.decodeStream(inputStream); // konversi urigambar menjadi image
