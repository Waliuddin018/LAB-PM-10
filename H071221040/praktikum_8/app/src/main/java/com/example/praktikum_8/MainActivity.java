...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadData(String query) { //memperbarui daftar notes dengan data yang sesuai dari basis data
        notes.clear(); //memastikan bahwa daftar hanya berisi catatan yang sesuai dengan kueri pencarian atau semua catatan jika kueri kosong
        Cursor cursor; //Deklarasi variabel cursor untuk menyimpan hasil kueri basis data
        if (query.isEmpty()) {
            cursor = dbConfig.getAllRecords(); //mendapatkan semua catatan dari tabel
        } else {
            cursor = dbConfig.searchByJudul(query); //mencari catatan yang sesuai dengan judul yang diberikan dalam kueri
        }
