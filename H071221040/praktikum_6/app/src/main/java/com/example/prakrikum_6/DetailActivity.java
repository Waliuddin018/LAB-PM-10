...
public class DetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra("userId")) { //memeriksa apakah ada data ekstrak tertentu(id) yg diterima
    ...
    private void getUserDetails(int userId) {
        Retrofit retrofit = RetrofitClient.getClient();//instance Retrofit yang dikonfigurasi
        ApiService apiService = retrofit.create(ApiService.class);// membuat instance ApiService, berdasarkan definisi dalam interface ApiService
        ...
        call.enqueue(new Callback<UserResponse>() {//callback Retrofit yang menangani respons dari pemanggilan API
            @Override
            public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                handler.post(() -> {
                    progressBar.setVisibility(View.GONE);
                    if (response.isSuccessful() && response.body() != null) {
                        //mengambil dan memproses data dari respons API dengan Retrofit
            ...
            @Override
            public void onFailure(Call<UserResponse> call, Throwable t) { //failure terjadi ketika pemanggilan API gagal
                handler.post(() -> {
                    progressBar.setVisibility(View.GONE);
                    showErrorView();
                });
