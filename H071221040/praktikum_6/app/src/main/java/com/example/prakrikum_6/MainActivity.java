...
public class MainActivity extends AppCompatActivity {
    ...
    private Executor executor = Executors.newSingleThreadExecutor();//untuk jelankan thread
    private Handler handler = new Handler(Looper.myLooper());
    private int currentPage = 1;//utk tampilkan 1 page di awal
    ...
    private void loadInitialData() {
        if (isNetworkAvailable()) {
            currentPage = 1; // memulai dari awal saat memuat data awal
            loadData();
        } else {
            showErrorView();
        }
    ...
    private void loadData() {
        progressBar.setVisibility(View.VISIBLE);
        Call<UserResponse> call = apiService.getUsers(currentPage, 6);//pemanggilan API untuk mendapatkan data pengguna
            ...
            public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    handler.post(() -> {
                        ...
                        userAdapter = new UserAdapter(users, context); //menampilkan daftar pengguna yang telah diambil dari API
